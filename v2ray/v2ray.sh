#!/bin/bash
# v2ray一键安装脚本
# Author: lonuslan<https://github.cm/lonuslan>
...
getCert() {
    ...
    if [[ -z ${CERT_FILE+x} ]]; then
        ...
        [[ -f ~/.acme.sh/$DOMAIN/ca.cer ]] || {
            colorEcho $RED " 获取证书失败，你可能需要自己查看报错信息来解决这个问题！"
            exit 1
        }
        CERT_FILE="/etc/v2ray/${DOMAIN}.pem"
        KEY_FILE="/etc/v2ray/${DOMAIN}.key"
...
menu() {
    ...
    echo -e "# ${GREEN}作者${PLAIN}: lonuslan                                      #"
    echo -e "# ${GREEN}网址${PLAIN}: https://github.com/lonuslan                                    #"
    echo "#############################################################"
